...
def clean_images():
	file_list = os.listdir('./ImagesDetect')
	for file_name in file_list:
		if '.png' in file_name:
			os.remove('./ImagesDetect/'+file_name)
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
       ...
       # cv2.imwrite(str(count)+'_'+text+'.png', sign)
        cv2.imwrite(os.path.join("./ImagesDetect/"+str(count)+'_'+text+'.png'), sign)
...
def main(frame):
    ...
    #NO Training phase - ARRUMAR - FORMATO YAML
    #model = np.fromfile('data_svm.dat', dtype=ml_StatModel)
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("camera/rgb/image_raw", Image, passa_video, queue_size= 1)
