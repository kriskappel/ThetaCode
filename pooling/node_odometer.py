...
def evaluateStop():
    ...
    if state == 'stop':
        stopMoviment()
    elif state == 'turnRight' and leftValue >= 400 and rightValue >= 400:
        print "TURN RIGHT : left value " + str(leftValue) + " right value " + str(rightValue)
        stopMoviment()
    elif state == 'turnLeft' and rightValue >= 400 and leftValue >= 400:
...
def startMovimentTurnRight():
    ...
    move.publish(100)
    #2.0
    pulses = rospy.Publisher('pulses', Int16, queue_size=1)
    pulses.publish(400)
